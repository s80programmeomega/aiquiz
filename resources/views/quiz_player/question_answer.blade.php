                    ...
                    @foreach ($data['question']->answers as $answer)
                        <label for="answer-{{ $answer->id }}"
                            ...
                            <span class="text-start lg:text-xl text-lg text-gray-800 peer-checked:text-[#ca77ff]">
                                @if (in_array($answer->type, ['text', 'emoji']))
                                    {{ $answer->title }}
                                @elseif($answer->type == 'photo')
                                    <img src="{{ Storage::disk(config('app.media_disk'))->url($answer->title) }}"
                                        class="max-h-40 mx-auto" alt="Uploaded Image">
                                @elseif($answer->type == 'audio')
                                    <audio controls>
                                        <source src="{{ Storage::disk(config('app.media_disk'))->url($answer->title) }}"
                                            type="audio/{{ pathinfo($answer->title, PATHINFO_EXTENSION) }}">
                                    </audio>
                                @elseif($answer->type == 'video')
                                    <video controls class="w-full max-h-40">
                                        <source src="{{ Storage::disk(config('app.media_disk'))->url($answer->title) }}"
