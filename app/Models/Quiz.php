                ...
                ->schema([
                    ...
                    Select::make('answer_type')
                        ...
                        ->afterStateUpdated(function ($state, Set $set, Get $get) {
                            ...
                            // Update the type field for each answer
                            foreach ($answers as $index => $answer) {
                                $answers[$index]['type'] = $state;
                            }
                    ...
                    Repeater::make('answers')
                        ...
                        ->validationAttribute(__('messages.common.answer'))
                        ->grid(2)
                        ->beforeStateDehydrated(function ($state, Get $get) {
                            // Final sync before saving
                            $answerType = $get('../../answer_type');
                            foreach ($state as $index => $item) {
                                $state[$index]['type'] = $answerType;
                            }
                            return $state;
                        })
                    ...
                    // })
                        ->schema([
                            Group::make([
                                Hidden::make('type')
                                    ->label('Answer Type')
                                    ->default(fn(Get $get): string => $get('../../answer_type'))
                                    ->required()
                                    ->columnSpan(3),
                                // Text input for text/emoji types
                                TextInput::make('title')
                                    ->placeholder(__('messages.common.answer'))
                                    ->label(__('messages.common.answer') . ':')
                                    ->required()
                                    ->columnSpan(3)
                                    ->visible(fn(Get $get) => in_array($get('../../answer_type'), ['text', 'emoji'])),

                                // File upload for photo, audio, and video types
                                FileUpload::make('title')
                                    ...
                                    ->acceptedFileTypes(function (Get $get) {
                                        ...
                                        } elseif ($answerType === 'audio') {
                                            return ['audio/mpeg', 'audio/wav', 'audio/ogg', 'audio/aac', 'audio/flac', 'audio/mp3', 'audio/m4a'];
                                        } elseif ($answerType === 'video') {
                                            return ['video/mp4', 'video/webm', 'video/ogg', 'video/mkv', 'video/avi'];
                                        }
                                        return [];
                                    })
                                    ->preserveFilenames()
                                    ...
                                    ->required()
                                    ->columnSpan(3)
                                    ->visible(fn(Get $get) => in_array($get('../../answer_type'), ['photo', 'audio', 'video'])),
